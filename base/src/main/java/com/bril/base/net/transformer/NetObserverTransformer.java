...
import com.bril.base.net.function.FunctionSwitch;
...
public class NetObserverTransformer<T> implements ObservableTransformer<BaseCallModel<T>, T> {

    @Override
    public ObservableSource<T> apply(Observable<BaseCallModel<T>> upstream) {
        return upstream.map(new FunctionSwitch<T>()).subscribeOn(Schedulers.newThread())
